...
OutputMap=function(
  ...
  MapRef_LR, ## spatial pixels data frame with spatial extent, automatically passed if not specified.
  ...
  names(InputDF)=c("Index_LR", "Month", "Values")
  ExpandedDF=merge(InputDF, PlotDF, all.y=TRUE); summary(ExpandedDF)
  ExpandedDF$Values[is.na(ExpandedDF$Values)]=DefaultVal
  
  #########################################################--
  
  Plot1_Agg=aggregate(Values~Index_LR+Month, ExpandedDF, AggregateFunction); ###summary(Plot1_Agg)
  Plot1_Wide=Long2Wide(Plot1_Agg, fName="Month", vName="Values", Prefix="m_"); ###summary(Plot1_Wide)
  names(Plot1_Wide)=c("Index_LR", MonthString); ###summary(Plot1_Wide)
  Plot1_Px=merge(MapRef_LR[ ,c("Index_LR")], Plot1_Wide); ###summary(Plot1_Px)
  ...
  if(PlotLog){
    ...
    Plot2Log_Wide=Long2Wide(Plot2_Agg[ ,c("Index_LR", "Month", "LogValues")],
                            fName="Month", vName="LogValues", Prefix="m_"); ###summary(Plot2Log_Wide)
    names(Plot2Log_Wide)=c("Index_LR", MonthString); ###summary(Plot2Log_Wide)
    Plot2_Px=merge(MapRef_LR[ ,c("Index_LR")], Plot2Log_Wide); ###summary(Plot2Log_Px)
