...
##outputs
  ...
  PrintMapsInHighResolution=FALSE,  ##New for 3.0.3 - Will give the option to print maps in high resolution
  WriteOutputCsv=TRUE,
  WriteMapSources=FALSE,
  WriteDetailedOutput=FALSE,
  PrintSummary=FALSE,  ##New for 3.0.3 - will allow user to see scenario summary in console after model run
  ...
  if(Fold) { ## fold spatial constraint 
    ...
    if(nrow(Constraints_Spatial)>0){
      ...
      for(i in 1:nrow(Constraints_Spatial)){
        ## constrain spatially
        MapRef_HR_CrI=MapRef_HR; ##mampref highres cropped I b/c it's going through the loop "i" times
        ...
        if(exists("MapRef_HR_Cr")){
          MapRef_HR_Cr=rbind(MapRef_HR_Cr, MapRef_HR_CrI)  ##Takes current object [i] and binds it to previous object
  ...
  if(Fold){ ## update spatial domain, save some output V2.2.4
    ...
    #V3.0.3- This selects the data to be used for mapping when a high or low resolution is run
    ##index is update if map resolution =/= model resolution
    if (PrintMapsInHighResolution == HighResolution){
      MapRef_ForMapping= MapRef
      UpdateIndex=FALSE
    }else if(  
      PrintMapsInHighResolution==FALSE & HighResolution==TRUE){
      MapRef_ForMapping= MapRef_LR
      names(MapRef_ForMapping)[names(MapRef_ForMapping)=="Index_LR"]="Index"  ##Changes column name as needed 
      UpdateIndex=TRUE
      }else{
      MapRef_ForMapping= MapRef 
      UpdateIndex=FALSE
      ...
      if(nrow(SC_GearCaps)>0){
        for(i in 1:nrow(SC_GearCaps)){
          ...
          if(any(is.na(Capped$Reduction))){
            print(paste("Error; Gear cap", i, "; some regions not matched to a reduction")); return(Capped);
          }
    ...
    if(TestScenario){
      ...
      if(nrow(SC_Closures)>0){
        ...
        for(i in 1:nrow(SC_Closures)){ ## for each closure
          print(paste("Defining Closure", i))
      ...
      if(Fold) { ## fold Scenario data 
        ...
        if(PrintScenarioMaps){
          ...
          if (PrintMapsInHighResolution){  ##new 3.0.3 pulls Index as denoted by high or low res maps
            Tmp=Stage4s[ ,c("VesselKey", "Month", "Index", "GearPerString", "StringsAtLength")];
          }else{
  ...
  if(Fold) {
    ...
    if(PrintDefaultMaps){
      ...
      if (PrintMapsInHighResolution){  ##mew 3.0.3 pulls Index as denoted by high or low res maps
        Tmp=Stage6d[ ,c("Month", "Index", "RopeStrength", "NumVerticalLinesAtStrength")];
        }else{
          Tmp=Stage6d[ ,c("Month", "Index_LR", "RopeStrength", "NumVerticalLinesAtStrength")];
        names(Tmp)[names(Tmp)=="Index_LR"]="Index"
      }
      Tmp=Stage6d[ ,c("Month", "Index", "RopeStrength", "NumVerticalLinesAtStrength")];
      Tmp2=aggregate(NumVerticalLinesAtStrength~Month+Index, Tmp, sum); ###summary(Tmp2)
      names(Tmp2)=c("Month", "Index", "Totals")
  ...
  if(Fold) { ## fold line strength management 
    ...
    if(TestScenario){
      ...
      if(PrintScenarioMaps){
        ...
        ##New for V3.0.3
        if (PrintMapsInHighResolution){  ##mew 3.0.3 pulls Index as denoted by high or low res maps
          Tmp=Stage7s[ ,c("Month", "Index", "RopeStrength", "NumVerticalLinesAtStrength")];
        }else{
  ...
  if(Fold) {
    ...
    if(PrintDefaultMaps){
      ...
      if(CoOccurrence){
        if (PrintMapsInHighResolution){  ##new 3.0.3 pulls Index as denoted by high or low res maps
          Tmp=Stage8d[Stage8d$ThreatMod=="CoOccurrence", c("Month", "Index", "NumVerticalLinesAtStrength", "Threat" )];
        }else{
        ...
        if (PrintMapsInHighResolution){  ##new 3.0.3 pulls Index as denoted by high or low res maps
          Tmp=Stage8d[Stage8d$ThreatMod=="Threat", c("Month", "Index", "NumVerticalLinesAtStrength", "Threat" )];
        }else{
      ...
      Tmp2=aggregate(Threat~Month+Index, Tmp, sum); ###summary(Tmp2)
      names(Tmp2)=c("Month", "Index", "Totals")
      Tmp=merge(Tmp, Tmp2); ###summary(Tmp)
    ...
    if(TestScenario){
      ...
      if(PrintScenarioMaps){
        ############# mean Threat #############################--
          if(CoOccurrence){
            if (PrintMapsInHighResolution){  ##mew 3.0.3 pulls Index as denoted by high or low res maps
              Tmp=Stage8s[Stage8s$ThreatMod=="CoOccurrence", c("Month", "Index", "NumVerticalLinesAtStrength", "Threat" )];
            }else{
            ...
            if (PrintMapsInHighResolution){  ##mew 3.0.3 pulls Index as denoted by high or low res maps
              Tmp=Stage8s[Stage8s$ThreatMod=="Threat", c("Month", "Index", "NumVerticalLinesAtStrength", "Threat" )];
            }else{
  ...
  if(Fold) {
    ...
    names(WhaleModel)=c("Month", "Index", "Index_LR", "WhaleDensity")
      ...
      if (PrintMapsInHighResolution){  ##new 3.0.3 pulls Index as denoted by high or low res maps
        WhaleModel_Map=WhaleModel[ c("Month", "Index", "Index", "WhaleDensity")] ## create copy for mapping 
      }else{
        WhaleModel_Map=WhaleModel[ c("Month", "Index_LR", "WhaleDensity")]
        names(WhaleModel_Map)[names(WhaleModel_Map)=="Index_LR"]="Index"
      }##ends if highresolution map loop
    ##
